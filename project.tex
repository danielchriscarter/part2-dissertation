...
The \textit{Service for User to Proxy} (S4U2proxy) system is an extension to the basic Kerberos setup that allows one service to obtain a ticket to another service (on behalf of a principal) in a controlled manner. Once a service has been marked as ``permitted'' to obtain tickets for another, it can do so simply by making a request to the KDC.
...
Note that this is \textbf{not} the same as S4U2self, which (if configured) effectively allows a server to bypass Kerberos authentication completely and obtain a ticket on behalf of a client, rather than being given a ticket by the client as part of the authentication process.

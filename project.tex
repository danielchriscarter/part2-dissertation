...
\begin{center}
  \begin{tikzpicture}[node distance=2.5cm]
    ...
    \draw [arrow, bend right] (webserver) edge node {\parbox{0.65\textwidth}{\textcircled{2} \\ \texttt{SELECT * FROM users WHERE \\ \hfill username = \textquotesingle\textquotesingle \ OR 1=1; -- \textquotesingle}}} (database);
...
\begin{center}
  \begin{tikzpicture}[node distance=2.2cm]
    ...
    \node (database) [trusted, below of=webserver] {Database};
    \node (kdc) [trusted, left of=user, xshift=-2cm] {KDC};
    ...
    \draw [arrow, bend right] (kdc) edge node[above, yshift=-0.5cm] {\textcircled{2} Kerberos ticket} (user);
...
\begin{tikzpicture}[node distance=3cm]
  \node[entity] (file) {File}
       ...
       child {node[attribute, left of=file] {id}}
...
\subsubsection{Database Setup Application}
It also includes a component which performs the necessary setup tasks (setting up SQL permissions), so that all that is required for the user of the application is to:
\begin{itemize}
\item
...
\section{System Structure and Repository Overview}
...
A full copy of the \verb+.htaccess+ file which configures many options in Apache for this project, is included in \hyperref[sec:appendix2]{Appendix 2}.
...
\section{Kerberos access to PostgreSQL}
...
\chapter*{Appendix 1: Django database router}
\label{sec:appendix1}
...
\chapter*{Appendix 2: Apache configuration}
\label{sec:appendix2}
